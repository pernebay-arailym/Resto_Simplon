...
def update_role(*, session: SessionDep, role_id: int, role_in: RoleUpdate):
    ...
    if not role_model:
        raise HTTPException(status_code=404, detail="Role not found")
    role_with_same_role_type = role_crud.get_role_by_role_type(session=session, role_type=role_in.role_type)
    if (role_with_same_role_type and (role_with_same_role_type.id != role_in.id)):
        raise HTTPException(status_code=400, detail="An other role with the same role type already exists.")
